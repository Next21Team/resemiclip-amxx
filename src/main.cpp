...
inline bool allowDontSolid(playermove_t *pm, edict_t *pHost, int host, int j)
{
	...
	if(bControllingByAMXX)
	{
		bool bIsSmbMasked = (pPlayer->GetMasked(IndexObject) || pObject->GetMasked(IndexPlayer));
		*pPlayer->GetSolid(pObject) = bIsSmbMasked;
		*pObject->GetSolid(pPlayer) = bIsSmbMasked;
	}
	else
	{
	...
	format << "allowDontSolid for " << IndexPlayer << ", object " << IndexObject << ", *pPlayer->GetSolid(pObject) = " << *pPlayer->GetSolid(pObject);
	format << " pPlayer->GetMasked(IndexObject) = " << pPlayer->GetMasked(IndexObject) << ", pObject->GetMasked(IndexPlayer) = " << pObject->GetMasked(IndexPlayer)
	<< ", m_iMaskPlayer " << pPlayer->GetMask() << ", m_iMaskObject " << pObject->GetMask() << "\n";
	g_engfuncs.pfnClientPrintf(EDICT(IndexPlayer), print_console, format.str().c_str());
	*/
...
void PM_Move(playermove_t *pm, int server)
{
	...
	if (g_Config.GetTime() && gpGlobals->time > g_Config.GetCount() && !bControllingByAMXX)
	{
